...
public class AlgRecPoli extends JFrame {
    ...
    AlgRecPoli() {
        this.setTitle("Recortes - Poligono");
...
class RecPoli extends JComponent {
    ...
    class Poly {
        ...
        Ponto2D vertexAt(int i) {
            return (Ponto2D) vp.elementAt(i);
        }
        ...
        void clipPoli(float xmin, float ymin, float xmax, float ymax) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                ppIns = pp.x <= xmax;
                if (piIns != ppIns) {
                    poly1.addVertex(new Ponto2D(xmax, pi.y + (pp.y - pi.y) * (xmax - pi.x) / (pp.x - pi.x)));
                }
                if (ppIns) {
                    poly1.addVertex(pp);
                }
            ...
            for (int i = 0; i < n; i++) {
                ...
                ppIns = pp.x >= xmin;
                if (piIns != ppIns) {
                    poly1.addVertex(new Ponto2D(xmin, pi.y + (pp.y - pi.y) * (xmin - pi.x) / (pp.x - pi.x)));
                }
                if (ppIns) {
                    poly1.addVertex(pp);
                }
            ...
            for (int i = 0; i < n; i++) {
                ...
                ppIns = pp.y <= ymax;
                if (piIns != ppIns) {
                    poly1.addVertex(new Ponto2D(pi.x + (pp.x - pi.x) * (ymax - pi.y) / (pp.y - pi.y), ymax));
                }
                if (ppIns) {
                    poly1.addVertex(pp);
                }
            ...
            for (int i = 0; i < n; i++) {
                ...
                ppIns = pp.y >= ymin;
                if (piIns != ppIns) {
                    poly1.addVertex(new Ponto2D(pi.x + (pp.x - pi.x) * (ymin - pi.y) / (pp.y - pi.y), ymin));
                }
                if (ppIns) {
                    poly1.addVertex(pp);
                }
